...
void oscEvent(OscMessage msg){
  ...
  band = 0;
  if(msg.checkAddrPattern("/muse/elements/alpha_session_score")){
    for(int ch = 0; ch < N_CHANNELS; ch++){
  ...
  band = 1;
  if(msg.checkAddrPattern("/muse/elements/beta_session_score")){
    for(int ch = 0; ch < N_CHANNELS; ch++){
      data = msg.get(ch).floatValue();
      buffer[band][ch][pointer[band]] = data;
    }
  ...
  band = 2;
  if(msg.checkAddrPattern("/muse/elements/gamma_session_score")){
    for(int ch = 0; ch < N_CHANNELS; ch++){
      data = msg.get(ch).floatValue();
      buffer[band][ch][pointer[band]] = data;
    }
  ...
  band = 3;
  if(msg.checkAddrPattern("/muse/elements/delta_session_score")){
    for(int ch = 0; ch < N_CHANNELS; ch++){
      data = msg.get(ch).floatValue();
      buffer[band][ch][pointer[band]] = data;
    }
  ...
  band = 4;
  if(msg.checkAddrPattern("/muse/elements/theta_session_score")){
    for(int ch = 0; ch < N_CHANNELS; ch++){
      data = msg.get(ch).floatValue();
      buffer[band][ch][pointer[band]] = data;
    }
